...
class OptimizationPlotter:
    def __init__(self, input_dir, output_file, metrics, lambda_reg=0.1):
        ...
        self.metric_functions = {
            ...
            'nll_poisson': self.calculate_nll_poisson,
    ...
    def calculate_nll_poisson(observed, df):
        trend = df['Trend'].values
        seasonals = [col for col in df.columns if 'Seasonal' in col]
        cycles = df[seasonals].sum(axis=1).values
        observed = df['Data'].values
        ...
        nll = -np.sum(observed * np.log(predicted) - predicted - gammaln(observed + 1))
        return nll
...
if __name__ == "__main__":
    input_dir = "../Output/RSV-Yu-2019/AnnualFixed-DualCycle/"
    output_file = "optimization_mesh"
    metrics = ['nll', 'mae', 'mae_reg', 'multiplicative', 'rank', 'autocorr']
    metrics = ['nll_']
    metrics = ['nll', 'nll_poisson', 'mae', 'mae_reg', 'multiplicative', 'rank', 'autocorr']
    metrics = ['nll_poisson']
